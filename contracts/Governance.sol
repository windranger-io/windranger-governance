    ...
    EIP712Upgradeable
{
    ...
    bytes32 private constant _EMPTY_ROLE = keccak256("EMPTY_ROLE");
    bytes32 private constant _DEVELOPER_ROLE = keccak256("DEVELOPER_ROLE");
    bytes32 private constant _LEGAL_ROLE = keccak256("LEGAL_ROLE");
    bytes32 private constant _TREASURY_ROLE = keccak256("TREASURY_ROLE");

    uint256 private constant _DEFAULT_PROPOSAL_THRESHOLD = 1e18;
    uint256 private constant _DEFAULT_ACTION_THRESHOLD = 1e18;
    ...
    modifier onlyGovernance() {
        require(_msgSender() == _executor(), "Governance: only governance");
        _;
    }

    modifier roleExists(bytes32 role) {
        require(_roles[role] > 0, "Governance: role doesn't exist");
        _;
    }
    ...
    function initialize(
        ...
        _timelock = timelock_;
        _roles[_TREASURY_ROLE] = 1;
        _roles[_DEVELOPER_ROLE] = 2;
        _roles[_LEGAL_ROLE] = 3;
        _rolesList.push(_TREASURY_ROLE);
        _rolesList.push(_DEVELOPER_ROLE);
        _rolesList.push(_LEGAL_ROLE);
        _proposalThresholds[_DEVELOPER_ROLE] = _DEFAULT_PROPOSAL_THRESHOLD;
        _proposalThresholds[_LEGAL_ROLE] = _DEFAULT_PROPOSAL_THRESHOLD;
        _proposalThresholds[_TREASURY_ROLE] = _DEFAULT_PROPOSAL_THRESHOLD;
    ...
    function setVotesOracle(address votesOracle_)
        ...
        onlyGovernance
    {
        require(votesOracle_ != address(0), "Governance: zero address");
        _openVotingOracle = IOpenVoting(votesOracle_);
        _roleVotingOracle = IRoleVoting(votesOracle_);
    }
    ...
    function registerRole(bytes32 role) external virtual onlyGovernance {
        require(_roles[role] == 0, "Governance: role exists");
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < voterRoles.length; ++i) {
            require(_roles[voterRoles[i]] > 0, "Governance: role exists");
            require(!hasRole(voterRoles[i], voter), "Governance: already set");
    ...
    function castVoteBySig(
        ...
        address voter = ECDSAUpgradeable.recover(
            _hashTypedDataV4(
                keccak256(abi.encode(_BALLOT_TYPEHASH, proposalId, support))
            ),
            v,
            r,
            s
    ...
    function propose(
        ...
        for (uint256 i = 0; i < targets.length; ++i) {
            if (targets[i] != address(this)) {
                ...
                require(registeredAction, "Governance: not registered");
            }
        }
        ...
        for (uint256 i = 0; i < signatures.length; ++i) {
            ...
            uint256 methodID = 0;
            require(calldatas[i].length >= 4, "Governance: length less than 4");
            for (uint256 j = 0; j < 4; ++j) {
                methodID = 256 * methodID + uint8(calldatas[i][j]);
            }
            require(sigID == methodID, "Governance: signatures mismatch");
        }

        Proposal storage proposal = _proposals[proposalId];
        require(proposal.voteStart.isUnset(), "Governance: already exists");
        require(
        ...
        returns (ProposalState)
    {
        ...
        if (proposal.executed) {
            ...
            revert("Governance: unknown proposal id");
        }
    }
    ...
    function _countVote(
        ...
        require(!receipt.hasVoted, "Governance: already cast");
        ...
        for (uint256 i = 0; i < proposal.roles.length; ++i) {
            if (
                ...
                if (support == uint8(VoteType.Against)) {
                    ...
                    revert("Governance: invalid vote type");
                }
            }
        }
    }
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < proposal.signatures.length; ++i) {
            ...
            if (quorum == 0) {
                quorum = _DEFAULT_PROPOSAL_THRESHOLD;
            }
